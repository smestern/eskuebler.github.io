//var dendriteColor = 0xffffff;
//var axonColor = 0xffffff;
...
var SharkViewer = function (parameters) {
	...
	*/		
	...
	this.annotation_color = 0x111111;
...
//generates sphere mesh
SharkViewer.prototype.generateAnnotation = function (a, radius) {
	var sphereMaterial = this.annotation_material;
	var r1 = (radius * 2) || 0.01;
	var geometry = new THREE.SphereGeometry(r1);
	var mesh = new THREE.Mesh(geometry, sphereMaterial);
	mesh.position.x = a.x;
	mesh.position.y = a.y;
...
SharkViewer.prototype.generateAnnotationParticle = function (a) {
	return new THREE.Vector3(a.x, a.y, a.z);
};

SharkViewer.prototype.createSphereImage = function() {
...
SharkViewer.prototype.avgRadii = function () {
	var radii_sum = 0;
	var radii_count = 0;
	for (var node in this.swc) {
		if (this.swc.hasOwnProperty(node)) {
			radii_sum += this.swc[node].radius;
			radii_count++;
		}
	}
